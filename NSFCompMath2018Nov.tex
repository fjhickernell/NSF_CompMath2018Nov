...
\begin{gather}  \tag{ERR-BD} \label{errorBd}
...
Algorithms satisfying \eqref{AlgErr} use approximations satisfying \eqref{errorBd} combined with a \emph{stopping criterion} that adaptively determines the necessary $n$.  $\ALG$ requires a \emph{data-driven error bound}, which depends on the definition of $\calc$ and is defined in terms of the function dara required for $\APP(f,n)$, to confidently bound the error:
...
As mentioned above, any algorithm that depends on function values may be fooled by a spiky function, even if that function has the regularity and any other nice properties guaranteed by the definition of the input space, $\calf$.  Therefore, our algorithms can only be guaranteed to succeed for some subset of the input space, $\calc$.

The de facto choice of many authors is to choose $\calc$ to be a ball of fixed radius, $\cb_{\rho} := \{f \in \calf : \norm[\calf]{f} \le \rho \}$.  Then one may define $\ALG$ satisfying \eqref{AlgErr} as follows:

\begin{algorithm}
	\caption{Model Ball\smallscoop-Based $\ALG$ \label{BallAlg}} 
	\begin{algorithmic}
	\PARAM a radius, $\rho$; $\APP$ satisfying \eqref{errorBd}
		\INPUT a black-box function, $f$; an absolute error tolerance,
		$\varepsilon>0$

\Ensure \eqref{AlgErr} for $\cc = \cb_{\rho}$
...
The parameters are the pieces that are essentially unchanged and defined by the algorithm designer.  The inputs are provided by the user and likely to change each time the algorithm is called.
This ball-based algorithm is \emph{non-adaptive} because $\ErrN$ does not depend on the function data, but only on the parameter $\rho$, which defines $\calc$.  It is unclear how $\rho$ should be chosen in practice.  

An \emph{adaptive} algorithm, whose computational cost depends on the input $f$ can be constructed for input functions whose norms are not much larger than their minimum norm interpolants.  Assume a sequence of nodes, $\designInf$, and let $f_{\textup{int},n}$, denote the minimum norm interpolant of $f \in \calf$ based on the data $\dataN$.  The following algorithm is defined to succeed for $\calc$ defined as a cone, 

\begin{algorithm}
	\caption{Prototype Cone-Based $\ALG$ \label{ConeAlg}} 
	\begin{algorithmic}
	\PARAM an initial sample size, $n_0$; an inflation factor, $\fC$; $\APP$ satisfying \eqref{errorBd}
		\INPUT a black-box function, $f$; an absolute error tolerance,
		$\varepsilon>0$

\Ensure \eqref{AlgErr} for  the cone $\cc = \bigl\{f \in \calf : \norm[\calf]{f} \le \inflate \norm[\calf]{f_{\textup{int},n_0}} \bigr \}$
...
Now the data-driven error bound, $\ErrN$, does depend on function data, so the algorithm is adaptive.  The set $\calc$ for which this algorithm succeeds is a \emph{cone}


One might imagine an algorithm that makes an initial choice of $n_0$ and uses the function data $\dataNo$ to compute the minimum norm interpolant, $\fint$.  Then one sets $\rho = \inflate \norm[\calf]{\fint}$ for some predetermined inflation factor $\fC$, and applies procedure \eqref{ballAlg}.  This algorithm satisfies the error criterion \eqref{AlgErr} for the \emph{cone} of functions 
\begin{equation}
    \calc = \bigl\{f \in \calf : \norm[\calf]{f} \le \inflate \norm[\calf]{\fint}, \text{ where } \fint \text{ is based on } \dataNo \bigr \}.
\end{equation}
This process is \emph{adaptive} because the $\rho$ in \eqref{ballAlg} is chosen based on function data.

To summarize, algorithms that succeed for balls, \smallscoop, 


then $\ALG(f,\varepsilon) = \APP(f,n')$ is the desired answer.


If $\norm[\calf]{\fint} \le \inflate \rho$ for some predetermined inflation factor $\fC$, then $\ALG(f,\varepsilon) = \APP(f,n')$ is the desired answer.  If not, raise $\rho$ to $\inflate \norm[\calf]{\fint}$ and iterate again. Such algorithm works for all $f$ in the cone of functions whose norm is not much larger than that of their minimium norm interpolant based on an initial sample.


the norm of this minimum norm interpolant is $\rho'$ , one might compute the minimum norm interpolant, 
