...
spikes fall  between the nodes as in Fig.\ \ref{quadfailfig}(a).  Worse, this $\herr_n(f)$ can fail 
for 
integrands that are \emph{not spiky} if $T_n(f)$ is coincidentally (nearly) 
the same as  $T_{n/2}(f)$ (see Fig.\ \ref{quadfailfig}(b)).  Over 30 years ago, James 
...
\begin{quote}
	...
	of apparently reasonable problems this will happen.
\end{quote}
Fig.\ \ref{quadfailfig}(b) depicts a reasonable integrand for which $\herr_n(f)$ in 
\eqref{tradtraperrest} fails.  Fig.\ 
